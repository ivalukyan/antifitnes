...
bearer_token = crm_env.bearer_token
user_token = ""
CID = crm_env.company_id

login = crm_env.login
password = crm_env.password
...
async def get_clients_ids(user_tok: Type[str]) -> Type[dict[int, str]]:
    ids = {}
    cnt = 1
...
async def get_info_about_users(list_id: Type[list], user_tok: Type[str], msg):
    sexes = {}
    names = {}
    phones = {}
    cnt = 1
...
async def update_db(total_count: int, msg) -> None:
    ...
    balances = await get_cards_loyalty(crm['user_token'], crm['ids'])
    crm['phones'] = result[0]
    crm['names'] = result[1]
    crm['sexes'] = result[2]
    # print(len(balances))
    if len(result) != 0:
        if len(result[0]) != 0 and len(result[1]) != 0 and len(result[2]) != 0:
            for _ in range(total_count, await get_total_count(crm['user_token'])-2):
                phone = result[0][_ + 1]
                name = result[1][_ + 1]
                sex = result[2][_ + 1]
                # print("upate cikl - %s" % _)
                balance = balances[_]
                # print("%s - balance - %s" % (_, balance))

                db_session = Session()
                profile = Profile(phone_number=phone, gender=sex, first_name=name, number_of_referral_points=balance)
...
async def get_cards_loyalty(user_token: Type[str], list_ids: Type[dict[int, str]]):
    ...
    for _ in range(len(list_ids)):
        url = f"https://api.yclients.com/api/v1/loyalty/client_cards/{list_ids[_ + 1]}"
...
async def update_without_db(msg):

    await msg.answer("Начало обновления! Not database.")
    ...
    if crm['ids'] is not None:
        ...
        crm['names'] = result[1]
        crm['sexes'] = result[2]
        await msg.answer("Обновление завершено! Not database.")
    else:
        await msg.answer("Ошибка обновления ids")